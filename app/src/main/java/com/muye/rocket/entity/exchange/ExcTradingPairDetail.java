 ...
 * 交易对详情
 */
public class ExcTradingPairDetail implements Serializable {
    ...
    public List<DepthItemEntity> getAskDepthItemEntity() {
        List<DepthItemEntity> depthItemEntityList = new ArrayList<>();
        if (getDepth() != null && getDepth().getAsks() != null && getDepth().getAsks().size() > 0) {
            List<DepthItemEntity> depthItemEntityList_ = buildDepthItemEntity(getDepth().getAsks());
            for (int i = depthItemEntityList_.size() - 1; i >= 0; i--) {
                depthItemEntityList.add(depthItemEntityList_.get(i));
            }
        }
        return depthItemEntityList;
    }

    public List<DepthItemEntity> getBidDepthItemEntity() {
        List<DepthItemEntity> depthItemEntityList = new ArrayList<>();
        if (getDepth() != null && getDepth().getBids() != null && getDepth().getBids().size() > 0) {
            List<DepthItemEntity> depthItemEntityList_ = buildDepthItemEntity(getDepth().getBids());
            for (int i = 0; i < depthItemEntityList_.size(); i++) {
                depthItemEntityList.add(depthItemEntityList_.get(i));
            }
        }
        return depthItemEntityList;
