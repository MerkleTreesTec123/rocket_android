...
public class RealNameStatusPresenter extends BasePresenter<RealNameStatusContract.View> implements RealNameStatusContract.Presenter {
    public RealNameStatusPresenter(RealNameStatusContract.View view) {
        super(view);
    }
    ...
    public void fetchReject() {
        mCompositeDisposable.add(
                mRetrofit.create(RApiService.class)
                        .getReason(MMKVTools.getToken())//方法
                        .compose(ResponseTransformer.handleResult())//处理返回结果
                        .compose(SchedulerProvider.getInstance().applySchedulers())//线程转换
                        .subscribeWith(new CustomResourceSubscriber<String>() {
                            ...
                            public void onError(ApiException exception) {
                                mView.onError(exception.getCode(), exception.getDisplayMessage());
                                mView.bindRealResult(null);
                            }

                            @Override
                            public void onNext(String reason) {
                                super.onNext(reason);
                                mView.bindRealResult(reason);
                            }
                        })
        );
    }
